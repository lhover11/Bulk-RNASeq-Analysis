...
topvar <- as_tibble(names(sort(topvar, decreasing=TRUE))[1:1000]) %>%
...
Trk_asrow <- as_tibble(gather(topvar, ID, logCPM, 2:ncol(topvar))) # gathers into three columns: row.names (genes), sample IDs and the logCPM values
Trk_asrow <- spread(Trk_asrow, Row.names, "logCPM") # spreads into a table where the genes are the columns and the samples are the rows
Trk_asrow <- column_to_rownames(Trk_asrow, var = "ID") # makes the row names the sample IDs so the data table contains only expression values
```


run PCA on samples and generates eigenvectors
```{r}
Sample_pca <- FactoMineR::PCA(Trk_asrow, scale.unit=TRUE, ncp=5, graph=TRUE)
ev <- Sample_pca$eig
```
...
```{r}
Sample_coord <- round(Sample_pca$ind$coord,4)
Sample_coord <- rownames_to_column(as_tibble(data.frame(Sample_coord)), "ID" )
...
```{r}
meta<-data.frame(row.names(Trk_asrow))
meta$Treat<-c(str_sub(meta$row.names.Trk_asrow.,1,2))  # Make new column with just EN or VH from each sample name
